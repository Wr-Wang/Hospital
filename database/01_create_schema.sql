/*
  ...
  创建数据库 + 全部表（合并自 000_init_database.sql + 010_wechat_login.sql）。
  共 29 张表：EF Core 模型 27 张业务表 + 微信登录表
    sec.WeChatAccounts      -- 患者微信账号关联
    sec.PatientRefreshTokens-- 患者 refresh_token
  表结构与 Hospital.Infrastructure 实体模型保持一致（EnsureCreated() 生成）。
  本脚本用于手动建库建表，或补建 EnsureCreated() 对已存在数据库不会增量添加的表。
